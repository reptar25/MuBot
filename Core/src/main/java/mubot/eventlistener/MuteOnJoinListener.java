...
public class MuteOnJoinListener implements EventListener<VoiceStateUpdateEvent> {
	 ...
	 * that guild
	 */
	public static HashMap<Snowflake, ArrayList<Snowflake>> mutedChannels = new HashMap<>();
	 ...
	 * @return the mono to check if a user should be muted on joining a channel
	 */
	 ...
	 * @return the mono to mute a given user
	 */
	 ...
	 * @return the mono to unmute a given user in a given guild
	 */
	private Mono<Void> unmuteUser(Mono<Member> memberMono, Mono<Guild> getGuild) {
		...
		return Mono.zip(memberMono, getGuild, getVoiceState).flatMap(tuple -> {
			...
			if (vs.isMuted() && !vs.getChannelId().orElseThrow().equals(guild.getAfkChannelId().orElseThrow())) {
