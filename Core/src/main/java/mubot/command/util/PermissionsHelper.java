...
import mubot.command.exceptions.BotPermissionException;
...
public final class PermissionsHelper {
    ...
    private static Mono<VoiceChannel> checkSameVoiceChannel(MessageCreateEvent event,
        ...
        return requireNotPrivateMessage(event)
                .then(Mono.zip(getBotVoiceChannelId, getUserVoiceChannelId).flatMap(tuple -> {
                    ...
                    // "!join !play"
                }).repeatWhenEmpty(MAX_RETRIES, Flux::repeat)).onErrorResume(IllegalStateException.class, error -> Mono.error(new CommandException("User and bot not in same channel",
                        "You have to be in the same voice channel as the bot to use this command"))).flatMap(event.getClient()::getChannelById).cast(VoiceChannel.class);
    }
    ...
    public static Mono<PermissionSet> requireBotChannelPermissions(GuildChannel channel,
        ...
        return channel.getEffectivePermissions(channel.getClient().getSelfId()).flatMap(permissions -> checkChannelPermissions(true, channel.getName(), permissions, requestedPermissions));
    }
    ...
    public static Mono<PermissionSet> requireMemberChannelPermissions(GuildChannel channel, Snowflake memberId,
                                                                      Permission... requestedPermissions) {
        return channel.getEffectivePermissions(memberId).flatMap(permissions -> checkChannelPermissions(false, channel.getName(), permissions, requestedPermissions));
    }
    ...
    public static Mono<PermissionSet> requireBotGuildPermissions(MessageCreateEvent event,
                                                                 Permission... requestedPermissions) {
        return event.getClient().getMemberById(event.getGuildId().orElseThrow(), event.getClient().getSelfId())
    ...
    public static String getChannelPermissionErrorMessage(boolean bot, Permission permission, String channelName) {
        ...
        sb.append(" in ").append(channelName);
        return sb.toString();
    }
}